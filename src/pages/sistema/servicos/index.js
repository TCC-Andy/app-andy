...
class Servico extends Component {
	...
	excluirServices = async (id) => {
		await api.delete(`/deleteService/${id}`).then(response => {
			if(response.data.status === 200){
				this.setState({
					success: response.data.mensagem
				})
											...
											(
												servicos.map(servico => (
													<tr key={servico._id}>
														...
														<td className="text-danger pencilTrash" onClick={() => { if (window.confirm(`Deseja DELETAR mesmo o serviço ${servico.nome}?`)) { this.excluirServices(servico._id) } }}><FaTrash /></td>
													</tr>
												))
											)
										}
