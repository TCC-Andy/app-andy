...
class Funcionario extends Component {
	...
	excluirFuncionarios = async (id) => {
		await api.delete(`/deleteEmp/${id}`).then(response => {
			if(response.data.status === 200){
				this.setState({
					success: response.data.mensagem
				});
	...
	render() {
		...
		const mensagem = this.state.success;
		const mensagemError = this.state.error;
		return (
			<div className="row">
											...
											(
												funcionarios.map(funcionarios => (
													<tr key={funcionarios._id}>
														...
														<td className="text-danger pencilTrash align-middle" onClick={() => { if (window.confirm(`Deseja DELETAR mesmo o funcionário ${funcionarios.nome}?`)) { this.excluirFuncionarios(funcionarios._id) } }}><FaTrash /></td>
													</tr>
												))
											)
										}
